...
test("server-renders the universal frontend-to-AI product", async () => {
  ...
  assert.match(html, /Current status, next action and recent activity/);
  assert.match(html, /title="Knowledge"/);
  assert.match(html, /title="Market Watch"/);
  assert.match(html, /title="Review Lab"/);
  assert.match(html, /title="Practice"/);
  ...
  assert.match(html, /ATLAS \/ PERSONAL OPS/);
...
test("v4 includes the task-first roadmap, evidence, interview and practice systems", async () => {
  ...
  assert.match(page, /type SiteTheme = "stealth" \| "career"/);
