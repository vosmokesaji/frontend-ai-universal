...
test("server-renders the universal frontend-to-AI product", async () => {
  ...
  assert.match(html, /dashboard\.tsx — frontend-ai-universal/);
  assert.match(html, /runtime overview · active queue · recent activity/);
  assert.match(html, /title="Knowledge Tree"/);
  assert.match(html, /title="Opportunity Log"/);
  assert.match(html, /title="Interview Console"/);
  assert.match(html, /title="Practice Lab"/);
  ...
  assert.match(html, /FRONTEND-AI-UNIVERSAL/);
  assert.doesNotMatch(html, />Frontend → AI</);
...
test("v4 includes the task-first roadmap, evidence, interview and practice systems", async () => {
  ...
  assert.match(page, /type SiteTheme = "career" \| "ide-native"/);
  assert.match(page, /Opportunity Log/);
